        ...
        if (keyIsDown(90)) {
            if (this.ShiftMode == true) {
                ...
                if (this.shotCountB >= this.shotInterval) {
                    if (this.Energy >= 240) {
                        this.bullets.push(new BulletB(new Vector(this.X + 10, this.Y - 6), new Vector(0, 20)));
                        this.bullets.push(new BulletB(new Vector(this.X - 10, this.Y - 6), new Vector(0, 20)));
                        this.shotCountB = 0;
                        this.Energy -= 240;
                        score -= 100;
                ...
                if (this.shotCountA >= this.shotInterval) {
                    if (this.Energy >= 30) {
                        this.bullets.push(new BulletA(new Vector(this.X, this.Y - 32), new Vector(0, 11)));
                        this.bullets.push(new BulletA(new Vector(this.X + 3, this.Y - 30), new Vector(2, 9)));
                        this.bullets.push(new BulletA(new Vector(this.X - 3, this.Y - 30), new Vector(-2, 9)));
                        this.shotCountA = 0;
                        this.Energy -= 30;
                        score -= 10;
...
class BulletB {
    ...
    draw() {
        ...
        quad(
          this.Position.X + 6, this.Position.Y,
...
class Enemies {
    ...
    createA() {
        this.enemies.push(new EnemyA(new Vector(random(40, 420), 0), new Vector(random(this.randomA), 1)));
    }
    createB() {
        this.enemies.push(new EnemyB(new Vector(random(60, 400), 0), new Vector(random(this.randomB), 1)));
    }
    createBoss() {
        this.enemies.push(new Boss(new Vector(230, -20), new Vector(0, 1)));
    }
    createRED() {
        this.enemies.push(new RED(new Vector(random(40, 420), 0), new Vector(random(-3, 3), 5)));
    }
    createBossBullets() {
...
class EnemyA {
    ...
    shot() {
        if (this.ShotCount > 60) {
            enemy.bullets.push(new EneBulletA(new Vector(this.Position.X, this.Position.Y), new Vector(2, 4), 10));
            enemy.bullets.push(new EneBulletA(new Vector(this.Position.X, this.Position.Y), new Vector(-2, 4), 10));
            this.ShotCount = 0;
        }
    }
...
class EnemyB {
    ...
    shot() {
        if (this.ShotCount > 45) {
            enemy.bullets.push(new EneBulletA(new Vector(this.Position.X, this.Position.Y), new Vector(2, 2), 8));
            enemy.bullets.push(new EneBulletA(new Vector(this.Position.X, this.Position.Y), new Vector(-2, 2), 8));
            enemy.bullets.push(new EneBulletA(new Vector(this.Position.X, this.Position.Y), new Vector(2, -2), 8));
            enemy.bullets.push(new EneBulletA(new Vector(this.Position.X, this.Position.Y), new Vector(-2, -2), 8));
            this.ShotCount = 0;
...
class Boss {
    ...
    draw() {
        ...
        fill(100)
        quad(
          this.Position.X + 30, this.Position.Y - 10,
          this.Position.X + 60, this.Position.Y - 30,
          this.Position.X + 70, this.Position.Y - 10,
          this.Position.X + 60, this.Position.Y + 40);
        quad(
          this.Position.X - 30, this.Position.Y - 10,
          this.Position.X - 60, this.Position.Y - 30,
          this.Position.X - 70, this.Position.Y - 10,
...
function updateGame() {
    ...
    for (var e of enemy.enemies) {
        for (var b of player.bullets) {
            if (sqrt(sq(b.Position.X - e.Position.X) + sq(b.Position.Y - e.Position.Y)) <= b.Size / 2 + e.Size / 2) {
                b.hit();
                e.damaged(b.Power);
            }
        }
        if (sqrt(sq(player.X - e.Position.X) + sq(player.Y - e.Position.Y)) <= player.Size / 2 + e.Size / 2) {
            if (player.InvincibleCount == 0) {
                player.damaged();
    ...
    for (var eb of enemy.bullets) {
        if (sqrt(sq(eb.Position.X - player.X) + sq(eb.Position.Y - player.Y)) <= eb.Size / 2 + player.Size / 2) {
            eb.hit();
            player.damaged();
        }
    }
